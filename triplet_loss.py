...
def model(X):
    ...
    with tf.variable_scope("model"):
        ...
        n_filter1 = 16
        kernel1 = 3
        conv1 = tf.layers.conv2d(X,n_filter1,kernel1,activation = tf.nn.relu,padding='SAME', reuse = True, kernel_initializer = tf.contrib.layers.xavier_initializer())    
    
        n_filter2 = 16     
        kernel2 = 3    
        conv2 = tf.layers.conv2d(conv1,n_filter2,kernel2,padding = 'SAME', kernel_initializer = tf.contrib.layers.xavier_initializer())
...
logits_anchor = model(X1)
logits_positive = model(X2)
logits_negative = model(X3)

margin = 100000

#d_positive = tf.norm(logits_anchor - logits_positive,axis=1)
#d_negative = tf.norm(logits_anchor - logits_negative,axis=1)


d_positive = tf.reduce_sum(logits_anchor - logits_positive,axis=1)**2
...
optimizer = tf.train.GradientDescentOptimizer(1e-14).minimize(reduced_loss)
