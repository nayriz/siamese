...
def cnn(X):
    ...
    conv1 = tf.layers.conv2d(X0,filter1,kernel1,activation = tf.nn.relu, padding = 'SAME',name='conv1')
    ...
    tf.summary.histogram('kernel1',kernel1)   
    tf.summary.histogram('bias1',bias1)   
    
    filter2 = 16
    kernel2 = 3

    conv2 = tf.layers.conv2d(conv1,filter2,kernel2, padding = 'SAME',name='conv2')
    ...
    tf.summary.histogram('kernel2',kernel2)   
